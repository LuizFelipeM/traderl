...
class TradingEnv(gym.Env):
    ...
    initial_balance: np.float64 = 10_000
    """
    Initial balance used for resets
    """

    balance: np.float64 = initial_balance
    """
    ...
    """
    Define if the end of the episode was reached
    """

    truncated: bool = False
    """
    Define if the episode ended because was out of bound
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=np.inf, shape=(6,), dtype=np.float64
        )
    ...
    @property
    def current_satoshi_price(self) -> np.float64:
        """
        Get the current close price in :const:`SATOSHI`
        """
        return np.float64(self.current_series["close"] / SATOSHI)

    @property
    def net_worth(self) -> np.float64:
        """
        Metric used fo the agent's performance. It can be calculated through:

        net_worth = :attr:`balance` + (:attr:`position` * current_price)
        """
        return np.float64(self.balance + (self.position * self.current_satoshi_price))
    ...
    def _buy(self) -> None:
        if self.balance > 0 and self.balance >= self.current_satoshi_price:
            ...
            self.balance -= np.float64(position * self.current_satoshi_price)

    def _sell(self) -> None:
        if self.position > 0:
            balance = np.float64(self.position * self.current_satoshi_price)
            # The real balance is calculated through the deduction of transactions fee from the full balance
            self.balance += np.float64(balance * (1 - self.transaction_fee.sell))
            self.position = 0
    ...
    def _next_observation(self) -> np.ndarray:
        ...
        Observations: [balance, position, low_price, high_price, open_price, close_price]
        """
        position = np.float64(self.position)
        if self.current_step >= len(self.data):
            self.done = True
            return np.array([self.balance, position, 0, 0, 0, 0])
